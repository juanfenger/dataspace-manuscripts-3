...
all_vars <- c("TCC","BCI","WCI","GCI","CY","BAH","BHSD",
              "SVF","Albedo","POP","FAR","MR", "Area","DIST")
...
# 导出为 8x6 英寸的 PDF
ggsave(
  "E:/第三篇论文/plot/Fig7_PCA_Analysis_Aligned-test.pdf", 
  plot = pca_fig, 
  device = "pdf", 
  width = 10, 
  height = 8, 
...
# 提取旋转后的载荷矩阵
rotated_loadings <- as.data.frame(rotated_result$loadings[1:14, 1:n_factors])
...
pca_data <- as.data.frame(pca_model$x[, 1:4])  # 提取前6个主成分
...
vote_result <- data.frame(
  Index = colnames(nb_all$Best.nc),
  Best_K = as.numeric(nb_all$Best.nc[1, ])
)
...
ggsave(
  "E:/第三篇论文/plot/xin/elbow_ptbiserial-test.pdf",
  p_pin,
  height = 12,
  width = 20,
  scale = 0.7
...
loadings_matrix <- as.data.frame(pca_model$rotation[, 1:4])
...
write_xlsx(cluster_character,"E:/第三篇论文/table/meanvalue-test.xlsx")
...
# 确保 pca_data 的列名是正确的
colnames(pca_data)[1:4] <- paste0("RC", 1:4)
...
write_xlsx(pca_cluster_profile,"E:/第三篇论文/table/PCA_meanvalue-test.xlsx")
# ---------------------- 导出合并结果 ----------------------
library(writexl)
library(dplyr)
...
# 4. 导出为新的 Excel 文件
# 建议加上时间或版本号，防止覆盖
output_file <- "E:/第三篇论文/table/xqsx4_Clustering_Results-test.xlsx"
